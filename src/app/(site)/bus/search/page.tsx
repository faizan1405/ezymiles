...
import { searchBusRoutes } from "@/server/transit";
import { BusResults } from "@/components/transit/bus-results";
  ...
  if (!from || !to) {
    return (
      <div className="container-page section-y text-center">
        <Bus className="mx-auto size-10 text-midnight-300" aria-hidden />
        ...
        <Button asChild variant="accent" className="mt-7">
          <Link href="/bus">Back to bus search</Link>
        </Button>
      </div>
    );
  }
        ...
        <div className="container-page">
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-4">
            ...
            <Button asChild variant="outline">
              <Link href="/bus">Change search</Link>
            </Button>
          </div>
        </div>
      </header>
...
async function Results({
  ...
  const routes = searchBusRoutes({ from, to });
  return <BusResults routes={routes} query={passthrough} />;
}
