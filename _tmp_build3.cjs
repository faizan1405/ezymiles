...
function extractPackages(content) {
  ...
  // Find the array content using bracket counting with string awareness
  let depth = 0;
  let inStr = false;
  let strChar = '';
  let arrayStart = -1;
  ...
  // Now extract each package using comment markers
  ...
  for (let i = 1; i < pkgBlocks.length; i++) {
    ...
    // Extract the complete object by finding matching braces
    ...
    // Replace destination: "" as never, with destinationSlug
    let transformed = objStr.replace(
      /^\s+destination:\s*""\s+as\s+never,/m,
      '    destinationSlug: "' + destSlug + '",'
    );

    // Add isDemoData if not present
...
const demoPkgs = extractPackages(demoContent);
...
for (const [slug, pkg] of seen) {
  idx++;
  if (idx <= 5) console.log('  ' + slug + ' -> ' + pkg.title);
}
