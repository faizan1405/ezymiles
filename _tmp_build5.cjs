...
function extractPackagesFromArray(content, arrayMarker) {
  const packages = [];
  const bounds = findArrayBounds(content, arrayMarker);
  if (!bounds) return packages;

  // Find all comment positions within the array content
  const commentPositions = findCommentPositions(bounds.content);

  for (const pos of commentPositions) {
    // pos is relative to bounds.contentStart
    const globalPos = bounds.start + arrayMarker.length - 1 + pos;
...
const demoPkgs = extractPackagesFromArray(demoContent, 'const packages = [');
console.log('Demo packages:', demoPkgs.length);

console.log('Extracting extra packages...');
